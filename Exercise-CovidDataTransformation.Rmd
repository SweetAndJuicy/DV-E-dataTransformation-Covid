...
#names_to names the columns with pivoted column names
#values_to names column with pivoted values
dc
```
...
dc <- dc %>%
  ...
  arrange(kommune, date_sample) %>%
  mutate(
    dcr100k = (casesDiagnosed/population)*100000,
...
# create new variables within mutate
# (e.g. 
# tested numbers per 100.000 people (tr100k)
...
dt <- dt %>%
  ...
  arrange(kommune, date_sample) %>%
  mutate(
    tr100k = (testsConducted/population)*100000,
